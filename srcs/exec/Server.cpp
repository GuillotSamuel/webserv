...
{
	for (size_t i = 0; i < this->tab_serv.size(); i++)
	{
		this->tab_list[i] = new ListeningSocket(this->tab_serv[i].getPort(), this->tab_serv[i]);
	}

	for (size_t i = 0; i < this->tab_serv.size(); i++)
	{
		this->_config[this->tab_list[i]] = &this->tab_serv[i];
	}
}
